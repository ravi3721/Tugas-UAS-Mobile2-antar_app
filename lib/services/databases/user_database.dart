...
class UserDatabase {
  ...
  List<antar_appUser> _userListFromSnapshot(QuerySnapshot? snapshot) {
    return snapshot!.docs.map((doc) {
      return antar_appUser(
          id: doc.id,
          nama: doc.get('nama') ?? '',
          email: doc.get('email') ?? '',
          foto: doc.get('foto'),
  ...
  // antar_appUser _userListFromSnapshot2(DocumentSnapshot? doc) {
  //   return antar_appUser(
  //       id: doc.id,
  //       nama: doc.get('nama') ?? '',
  //       email: doc.get('email') ?? '',
  ...
  Stream<List<antar_appUser>> get user {
    return userCollection.snapshots().map(_userListFromSnapshot);
    // return userCollection.doc(uid).snapshots().map(_userListFromSnapshot2);
  }

  Stream<antar_appUser> get user2 {
    // return userCollection.snapshots().map(_userListFromSnapshot);
    return userCollection.doc(uid).snapshots().map((event) => antar_appUser(
        id: event.id,
        nama: event.get('nama'),
